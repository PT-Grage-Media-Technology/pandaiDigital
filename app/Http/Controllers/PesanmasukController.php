...
class PesanmasukController extends Controller
{
    ...
    public function sendReply(Request $request, string $id): RedirectResponse
    {
        ...
        // Kirim email ke alamat email yang dituju
        $to = $pesan->email;
        $subject = $pesan->subjek;
        $headers = "From: admin@gmt.web.id\r\n";
        $headers .= "Reply-To: admin@gmt.web.id\r\n";
        $headers .= "Content-Type: text/html; charset=UTF-8\r\n";

        mail($to, $subject, $emailContent, $headers);

        Log::info('Email sent to: ' . $to); // Logging untuk debugging

        return redirect()->route('administrator.pesanmasuk.show', $id)->with('success', 'Email sukses terkirim ke ' . $pesan->email);
    }
