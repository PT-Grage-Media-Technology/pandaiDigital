...
class VideoController extends Controller
{
    ...
    public function index(Request $request):View
    {
        ...
        if (!empty($search)) {
            $query->where('jdl_video', 'like', "%$search%");
        }
    ...
    public function store(Request $request)
    {
        ...
        if($request->hasFile('video')) {
            ...
            $videoName = null; // Menambahkan ini untuk memastikan $videoName adalah null jika tidak ada file
        }
    ...
    public function update(Request $request, string $id_video)
    {
        ...
        if ($request->hasFile('gbr_video')) {
            ...
            // Pindahkan gambar baru ke direktori public_html/foto_video
            $gbr_video->move(base_path('public_html/foto_video'), $gambarName); 
        
            // Cek apakah gambar lama ada, jika ada, hapus
            if ($videos->gbr_video && file_exists(base_path('public_html/foto_video/' . $videos->gbr_video))) {
                unlink(base_path('public_html/foto_video/' . $videos->gbr_video));
            }
        ...
        if ($request->hasFile('video')) {
            $video = $request->file("video");
            $videoName = $video->getClientOriginalName();
            $video->move(base_path('public_html/video_files'), $videoName); // Pastikan direktori penyimpanan benar
        
            // Setelah video baru berhasil diunggah, hapus video lama
            if ($videos->video && file_exists(public_path('video_files/' . $videos->video))) {
                unlink(base_path('public_html/video_files/' . $videos->video));
            }
        
            // Update nama video di database
    ...
    public function destroy(string $id_video)
    {
        ...
        if ($videos->gbr_video) {
            $path = base_path('public_html/foto_video/' . $videos->gbr_video);
            if (file_exists($path)) {
                unlink($path);
            }
        }
        ...
        if ($videos->video) {
            $path = base_path('public_html/video_files/' . $videos->video);
            if (file_exists($path)) {
                unlink($path);
            }
        }
