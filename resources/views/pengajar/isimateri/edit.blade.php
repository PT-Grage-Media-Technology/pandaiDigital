                ...
                <div class="card-body">
                    <form action="{{ route('pengajar.isimateri.update', $isi_materis->id_isi_materi) }}" method="POST" enctype="multipart/form-data"
                        ...
                        <table class="table" id="datatable-buttons" style="border: none; border-collapse: collapse;">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 5px;">
                                         @if (strpos($isi_materis->file, '.mp4') !== false ||
                                                    strpos($isi_materis->file, '.avi') !== false ||
                                                    strpos($isi_materis->file, '.mpeg') !== false)
                                                <video width="100" controls>
                                                    <source src="{{ asset('files/' . $isi_materis->file) }}"
                                                        type="video/mp4">
                                                    Your browser does not support the video tag.
                                                </video>
                                            @elseif (strpos($isi_materis->file, '.jpg') !== false ||
                                                    strpos($isi_materis->file, '.jpeg') !== false ||
                                                    strpos($isi_materis->file, '.png') !== false ||
                                                    strpos($isi_materis->file, '.gif') !== false)
                                                <img src="{{ asset('files/' . $isi_materis->file) }}" alt="Image"
                                                    width="100">
                                            @elseif (strpos($isi_materis->file, '.pdf') !== false)
                                                <a href="{{ asset('files/' . $isi_materis->file) }}" target="_blank">Lihat
                                                    PDF</a>
                                            @else
                                                <span>Tidak ada preview</span>
                                            @endif
                                        <input type="file" class="form-control" id="file" name="file">
                                         <p class="text-xs">Hanya bisa file dengan tipe mp4, avi, jpg, jpeg, png, pdf</p>
                                    </td>
                        ...
                        <div class="mt-4 d-flex justify-content-between">
                            ...
                            <a href="{{ route('pengajar.isimateri.index', ['id_materi' => $isi_materis->id_materi]) }}" class="btn btn-danger">Batal</a>
                        </div>
                    </form>
                </div>
            </div>
